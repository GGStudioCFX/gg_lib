...
GGVehicle = GGVehicle or {}

local gameBuild = GetGameBuildNumber()

function GGVehicle.getProperties(vehicle)
...
local setLockState = GetConvarBool("gg:setLockState", false)

function GGVehicle.setProperties(vehicle, props, fixVehicle)
    if not DoesEntityExist(vehicle) then
        error(("GGVehicle.setProperties: vehicle '%s' does not exist"):format(tostring(vehicle)))
    end
    ...
    if props.lockState ~= nil and setLockState then SetVehicleDoorsLocked(vehicle, props.lockState) end
    ...
    if props.tyres then
        for tyre, state in pairs(props.tyres) do
            if state == 3 then
                BreakOffVehicleWheel(vehicle, tonumber(tyre), false, true, true, false)
            else
